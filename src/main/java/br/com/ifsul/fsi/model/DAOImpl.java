package br.com.ifsul.fsi.model;
...
public abstract class DAOImpl<T, I extends Serializable> implements DAO<T, I> {
    ...
    // Retirar TRY CATCH e colocar throws Exeception aquuie na classe acima
    ...
    public void delete(Class<T> classe, String id) {
        try {
            EntityManager em = this.getEntityManager();
            em.getTransaction().begin();
            Query query = em.createQuery("delete from " + classe.getSimpleName() + " o where o.usuario = :id");
            query.setParameter("id", id);
            query.executeUpdate();
            em.flush();
            em.getTransaction().commit();
